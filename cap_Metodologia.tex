...
Porém, \v{Z}i\v{z}lavsk\'{y} destaca algumas limitações, muitas vezes subestimadas, do VPL como ferramenta de análise de oportunidades de inovação\footnote{Ver \cite{ziz2014}}:
\begin{itemize}
...
A questão a ser discutida agora é: como se calcula o VPL? Gallo cita que, por mais que não se calcule essa função na mão, é útil entender a matemática por trás dela; a Definição \ref{VPLdef} é um esboço matemático dos princípios que regem o VPL\footnote{Ver \cite{SITE:NPV}}.
...
\begin{equation}\label{eq:pp1}
	\mathbf{p_W^{(n+1)}} = \mathbf{p_W^{(n)}} \left(1 \pm N\delta_W \right),
\end{equation}
...
\begin{enumerate}
	...
	\item Para cada vetor de pressão obtido, simule o reservatório e calcule o VPL modificado com a Equação \eqref{eq:ModWellVPLDef} para cada poço;
...
Outros adendos podem ser feitos ao \textit{Smart Reservoir}; contudo, no âmbito dessa pesquisa, apenas os adicionais citados acima foram incluídos no algoritmo. Uma sugestão para trabalhos futuros é a implementação de novas abordagens a serem adicionadas ao algoritmo proposto com vistas à economia de custos computacionais, redução de custos de produção e redução no tempo de explotação.
...
no tempo total de execução resulta em uma grande economia de recursos computacionais, evidenciando a evidente participação das técnicas paralelas na redução de tempo de produção e custos computacionais almejada pela utilização do \textit{Smart Reservoir}. 