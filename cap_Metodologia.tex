...
Até o momento, o \textit{Smart Reservoir} apenas examina as pressões nos poços produtores; porém, a partir de um determinado instante na produção, o NPV normal calculado passa a ser inferior ao custo de injeção de água; neste caso, um primeiro adendo ao algoritmo mostrado na Seção \ref{subsec:SR} seria a redução das vazões de injeção de água quando as mesmas se tornam superiores ao valor do NPV normal calculado. Essa redução pode ser dada, por exemplo, de duas formas:
\begin{itemize}
...
O segundo passo adicionado ao \textit{Smart Reservoir} foi o uso de \textit{janelas de predição}: no passo de exame das pressões nos produtores, foi considerada, ao invés de uma janela de tempo apenas, a existência de uma segunda janela para cálculo do NPV modificado, em que as pressões consideradas são da forma $\mathbf{p_W^{(n+1)}} = \mathbf{p_W^{(n)}} \pm 2N\delta_W,$ em que $N$ é definido pela Equação \eqref{eq:pN}; no caso da primeira iteração, as pressões escolhidas para a janela de predição são calculadas pela relação $\mathbf{p_W^{(1)}} = \mathbf{p_W^{(0)}} - 2i\delta_W,~~i = 0,\ldots,n_{bhp}-1$. A premissa do uso da janela de predição é a do \textit{Smart Reservoir}, no momento em que calcula o NPV modificado, tentar prever, em um passo a frente, qual o melhor caminho de pressões para os poços produtores deve ser feito; essa ideia é baseada, em uma abordagem simples, em um Modelo de Controle Preditivo (MPC) com uma janela de predição de dois passos\footnote{Detalhes sobre o MPC podem ser encontrados em \cite{garcia1989model}}.

A última adição ao \textit{Smart Reservoir} a ser discutida está relacionada ao próprio cálculo do NPV Modificado; a Equação \eqref{eq:ModWellNPVDef} possui um comportamento numericamente indesejável quando a variação de saturação é muito pequena ou nula --- a depender das limitações da representação de dados no simulador, por exemplo, o NPV modificado resultante pode ser considerado infinito para algumas variações de saturação. Para evitar que isso ocorra, um valor positivo $b$ pode ser adicionado à variação de saturação, para que se garanta que o NPV modificado assuma um valor finito. A Equação \eqref{eq:ModWellNPVDef}, portanto, pode ser redefinida como

\begin{equation}\label{eq:ModWellNPVDef2}
\widetilde{NPV}(s,P) = \int_{s_0}^{s_f} \frac{C_F(s,P)}{\dot{s} + b}ds,
\end{equation}
