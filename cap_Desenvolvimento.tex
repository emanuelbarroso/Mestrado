...
\begin{tabular}{|c|c|c|}
	\hline
	\textbf{Simulação} & \textbf{NPV }$\mathbf{(\times10^6\text{ USD})}$ & \textbf{Passos de tempo (90 dias)} \\ \hline
...
Analisando-se a Tabela \ref{tab:EggAnalysis}, pode-se afirmar que há uma progressão nos valores de NPV à medida em que se prossegue com os ajustes na taxa de variação das pressões e se adiciona passos extras ao \textit{Smart Reservoir}. Na sua implementação mais simples, sem a janela de predição, há uma sensível melhoria do NPV, mas muito pequena para considerar a utilização do algoritmo. A adição da janela de precisão, por si só, também contribui, porém a variação é também insuficiente para justificar o uso do algoritmo.

O ganho com NPV passa a ser notado na fase 3 da simulação, em que a taxa de variação das pressões é elevada em relação às fases anteriores; isto foi feito de modo a se testar os limites de estabilidade numérica do algoritmo. Na fase 4, já se considera a taxa variável dependente da saturação; o NPV resultante é consideravelmente maior do que nas fases anteriores, chegando próximo aos patamares desejados\footnote{Esse patamar é o resultado simulado do algoritmo ssStoSAG, proposto em \cite{fonseca}.}; contudo, a fase 4 é incompleta no sentido de que não há o fator de correção numérica, fato esse que, em determinados casos, contribui para inconsistências no cálculo do NPV modificado. A adição desse fator, aliada a uma nova elevação das taxas de variação de pressão, contribui para um incremento ainda maior do NPV, como pode ser visto observando-se o resultado da fase 5. 

Ao final das simulações, não somente se obteve um NPV equivalente ao encontrado utilizando-se o algoritmo proposto por Fonseca \textit{et al.}, como também foi possível reduzir o tempo de simulação para 40\% do original. Essa redução se faz interessante se for feita a consideração de que, em situações reais, o NPV é afetado por um fator de desconto ao longo do tempo\footnote{Ver \cite{ziz2014}}. Além da vantagem de se reduzir o tempo em termos de análise econômica, tem o fato de que os custos computacionais são significativamente reduzidos; uma simulação de \textit{Smart Reservoir} com o \textit{Egg Model}, na fase 5, é equivalente, em termos computacionais, a 15 simulações normais, cada uma durando 4 anos. Para se ter uma ideia da magnitude dessa economia de custo computacional, Fonseca \textit{et al.} obtêm seu valor de NPV após 4000 simulações \cite{fonseca}; portanto, o uso do \textit{Smart Reservoir} representou uma economia de custo de mais de 95\%, mantendo um NPV próximo à meta e reduzindo o tempo de produção.
...
Em termos de NPV e tempo de explotação, em ambos os casos do SAIGUP o NPV resultante pode ser considerado equivalente nas situações normal e com a implementação do \textit{Smart Reservoir}; no entanto, percebe-se uma drástica redução no tempo total de simulação em ambos os casos: para aproximadamente 37.5\% do tempo original para o caso de curto prazo e para menos de 35\% do tempo original no caso de longo prazo, confirmando, assim, o cumprimento da ideia central da solução proposta. A Tabela \ref{tab:SAIGUPAnalysis}. apresenta os resultados obtidos com o SAIGUP. Além da economia de tempo e a obtenção do NPV semelhante ao desejado, percebe-se que, particularmente no SAIGUP de longo prazo, a simulação com o \textit{Smart Reservoir} equivale a uma simulação normal de 4 anos e meio.
...
\begin{tabular}{|c|c|c|c|}
	\hline
	\textbf{Simulação} & \textbf{NPV }$\mathbf{(\times10^9\text{ USD})}$ & \textbf{Passos de tempo} & \textbf{Comprimento do passo} \\ \hline
